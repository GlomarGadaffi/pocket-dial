...
// (MAX_CLIENTS + MAX_SUBSCRIPTIONS + headroom, Issue #54) so peak fan-out fits.
// Since #409 no pool allocates when it runs dry: exhaustion REFUSES, so each size
// below is a hard capacity, not a soft target with a heap overflow behind it.
//
...
// keeps the broadcast+NOTIFY peak inside the pool. Past it, getMessageFromPool()
// returns nullptr and the caller drops the packet (SIP over UDP retransmits, so a
// drop costs latency, not the call; a 503 is impossible because building one
// needs a message from the same empty pool). Override to claw back RAM on a
// constrained node -- a smaller pool means earlier drops, never heap use.
#ifndef POCKETDIAL_MSG_POOL
#define POCKETDIAL_MSG_POOL (POCKETDIAL_MAX_CLIENTS + POCKETDIAL_MAX_SUBSCRIPTIONS + 4)
#endif

// Issue #409: the #101(A) heap fallbacks behind the message and virtual-peer
// pools are GONE, and so are their ceilings. A build that still sets either
// knob is relying on behaviour that no longer exists -- fail it loudly rather
// than silently ignore the flag.
#if defined(POCKETDIAL_MSG_HEAP_FALLBACK_MAX) || defined(POCKETDIAL_VPEER_HEAP_FALLBACK_MAX)
#error "POCKETDIAL_*_HEAP_FALLBACK_MAX were removed by #409: pools refuse when empty and never heap-allocate. Drop the flag; size the pool instead."
#endif
