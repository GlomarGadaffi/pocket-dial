...
| `TOUCH_RST` | **not routed** | N/A | **Corrected.** The AXS15231B on this board has no reset line brought out; the driver is initialised with `rst = GPIO_NUM_NC` (`main/esp_main_display.cpp:59-64`). |
| `TOUCH_INT` | **not routed** | N/A | **Corrected.** No touch interrupt either: `int = GPIO_NUM_NC`, and **touch is polled**. |
...
microSD/TF slot on a **separate SPI bus** (`SPI3_HOST`; the W5500 owns `SPI2_HOST`), so the
...
A legacy ESP32-WROVER-E board with Power-over-Ethernet (PoE) and RMII-based LAN8720 Ethernet. It uses the native ESP32 internal MAC.
