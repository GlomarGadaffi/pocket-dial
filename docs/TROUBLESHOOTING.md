...
> this box said it did not, claiming the erase was issued on the `storage` namespace while
...
| Cause | Fix |
| :--- | :--- |
| **Connection refused** | **A real fault now.** The listener accepts unconditionally in every provisioning state; there is no dark/open gate and nothing to "reopen". Check you are at the right IP and that nothing on your machine is filtering port 80. (`http_dashboard` is a FreeRTOS **task name**, on `eth`/`lan8720` only; the `wifi` build calls it `http_server_task`. Neither is printed at boot, so do not wait for a log line.) |
