...
> (`HttpServer.cpp:1741`), a deliberate decision rather than an oversight, so
...
network, the server applies Same-Origin Verification to **every gated route**. That covers every
...
design, that is the same floor a lost sysop credential already sits on.
...
```json
{
  ...
  "minFreeHeapInternal": 41208,
  "freeHeapInternal": 58336,
  "largestFreeBlockInternal": 21492,
  "freeHeapDma": 58336,
  "largestFreeBlockDma": 21492,
  "resetReason": "POWERON",
...
| `minFreeHeapInternal` | Integer | `heap_caps_get_minimum_free_size(MALLOC_CAP_INTERNAL)` (#273/#275), the LOWEST internal-DRAM level seen since boot. Exists because neither figure above can show a DRAM shortage: `MALLOC_CAP_SPIRAM` is PSRAM by definition, and the all-caps minimum is so dominated by 8 MB of PSRAM that a near-exhausted 320 KB of DRAM barely moves it (measured on a real board: `minFreeHeap` 2516936 vs `minFreeHeapSpiram` 2512392 -- a 4.5 KB gap across a whole boot). Task stacks and lwIP pbufs come from here and nowhere else. **Read it as a watermark, not a gauge** -- see the two fields below. `0` on the host build. |
| `freeHeapInternal` | Integer | `heap_caps_get_free_size(MALLOC_CAP_INTERNAL)` (#273), total internal DRAM free **right now**. The companion the watermark above needs: a low-water mark is monotonically non-increasing, so it can never distinguish a slow leak from one large transient dip, and on #273 it produced exactly that wrong reading (a drop across a window containing both quiet time and five `/api/status` requests was read as background drain, when one DMA bounce-buffer allocation explains it equally well). Trending down across quiet windows is a leak; returning to its prior level after a spike is not. `0` on the host build. |
| `largestFreeBlockInternal` | Integer | `heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL)` (#273), the biggest **contiguous** internal-DRAM block right now. Reported separately from total free because the allocation that fails on this board is contiguous *and* aligned: `spicommon_dma_setup_priv_buffer()` ends in `heap_caps_aligned_alloc(alignment, align_len, mem_cap)`. So total free can look comfortable while no single block is large enough. **A widening gap between this and `freeHeapInternal` is fragmentation; both falling together is exhaustion.** Neither this field nor the docs decide which is happening -- reporting both is what lets a reader find out. `0` on the host build. |
| `freeHeapDma` / `largestFreeBlockDma` | Integer | The same two readings for `MALLOC_CAP_DMA` (#273). Present alongside the `Internal` pair rather than instead of it because the IDF call above takes its caps from the **caller** (`mem_cap` is a parameter), so `MALLOC_CAP_INTERNAL` is a proxy for the failing pool, not the pool itself. On ESP32-S3 the two capability sets are nearly identical; "nearly" is exactly the kind of assumption that has already produced wrong conclusions on #273, so both are measured. If these two ever diverge meaningfully from the `Internal` pair, trust these -- they are the ones the failing allocation actually draws from. `0` on the host build. |
...
notice with. Resend with `confirm=LOCKOUT` to override; it is the same shape as
...
Captured packets are synthesized into a minimal Ethernet+IPv4+UDP frame around the exact SIP bytes (dummy MAC addresses, since the server has no real link-layer information, but real source/destination IP:port), so Wireshark's SIP dissector decodes them exactly as it would a real capture. Timestamps are relative to the server's monotonic clock, not wall-clock (no RTC is guaranteed on the device; same basis as `/api/cdr`'s `startMs`).
...
* Auth: fully gated with `needCsrf = false`, the same gate as `/api/pcap` (gates 1, 2 and 4; no token), because it is the same underlying data.
* Request Headers: `Cookie: pd_session=…` (required)
* Response Content-Type: `application/json`
* Response Status Codes:
...
Host/desktop build, where the wipe completes but there is no firmware to restart:
```json
{
  "status": "ok",
